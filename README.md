...
* sequence editor now supports use of the cursor keys to move selected notes up/down in pitch or left/right in time.  Up and down move by one semitone.  Hold down Shift to move up or down by octave.  Ctrl-Right and Ctrl-left move the selected notes by the current snap amount.  (Note that it doesn't automatically snap the notes to the grid; if you want that, use the quantize function first.)  You can also nudge the selected notes by individual ticks by holding down the Shift key with left or right.
...
* NEW - Step edit is added on record, starting at transport line. Sequence editor right and left key moves transport line by snap. Home key moves transport line to start. 
